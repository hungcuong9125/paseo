...
export function createAntigravityACPThinkingBridge(
  ...
  return {
    ...
    thinkingOptionWriter: async (context) => {
      const model = context.currentModelId ? modelsById.get(context.currentModelId) : undefined;
      if (!model?.thinkingOptions?.length) {
        throw new Error("Antigravity thinking requires a configured model with thinkingOptions");
      }
      if (
        !model.thinkingOptions.some((option) => option.id === context.requestedThinkingOptionId)
      ) {
        throw new Error(
          `Unknown Antigravity thinking option "${context.requestedThinkingOptionId}" for model "${model.id}". Allowed: ${model.thinkingOptions
      ...
      await selectAntigravityModelVariant({
        ...
        configOptions: context.configOptions,
        modelId: model.id,
        thinkingOptionId: context.requestedThinkingOptionId,
      });
    },
  };
}
